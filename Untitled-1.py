...
class machine_details(Tk):
    ...
    def newcodesaved(self):
        if self.editdivval.get() == '':
            ...
            cursor.execute('update Machine_Details set Division = ?, Machine_Name=?, Capacity=?, Machine_Code=?, No_of_Machines=?, Serial_No=?, Purchased_Date=?, Supplier_Name=?, Final_Machine_Code=? where Final_Machine_Code=?',(Division, Machine_Name, Capacity, Machine_Code, No_of_Machines, Serial_No, Purchased_Date, Supplier_Name, Final_Machine_Code, Old_Code))
            conn.commit()
